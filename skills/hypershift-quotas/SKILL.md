...
The `check-quotas.sh` script lives in `../hypershift-cluster/scripts/`.
...
../hypershift-cluster/scripts/check-quotas.sh
```
...
../hypershift-cluster/scripts/check-quotas.sh --request-increases
```
...
Recommended warnings show when quota is below the suggested minimum.
...
aws ec2 describe-vpcs --query 'Vpcs | length(@)'
../hypershift-cluster/scripts/check-quotas.sh | grep -A2 "VPC Resources"
