...
# Best-effort credential load. Debug tolerates missing creds (it reports what it
# can), so unlike create/destroy this only sources a .env file if one is found and
# creds are not already in the environment; it never exits on absence.
...
# Colors come from lib-common.sh (sourced above). This script uses bespoke,
# CHECK_MODE-aware log helpers (and its own [ERROR]-prefixed log_error, which
# intentionally overrides the lib's). All return 0 to avoid set -e issues.
